...
alter table if exists bucket_configs enable row level security;
alter table if exists income_entries enable row level security;
alter table if exists expenses enable row level security;
alter table if exists bucket_transactions enable row level security;
alter table if exists weekly_snapshots enable row level security;
...
-- Create unique constraint for weekly snapshots
...
BEGIN
  ...
  IF v_avg_weekly_income IS NULL OR v_avg_weekly_income = 0 THEN
    v_score := 50; -- Neutral score for new users
  ELSE
